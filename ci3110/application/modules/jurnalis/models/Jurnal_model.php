<?php
    ...
    class Jurnal_model extends CI_Model
    {
        ...
        function get_jurnal_today(){
          $table=$this->table;
          $sql=$this->db->query("SELECT DISTINCT dokumen.* FROM dokumen,operator WHERE dokumen.`Tgl_Out_Verif`=CURRENT_DATE() AND position='verifikasi1'");
          return $sql->num_rows();
        }
        ...
        function get_data_search(){
          ...
          if ((empty($start)) && (empty($end))) {
            ...
            $sql=$this->db->query("SELECT * FROM dokumen where Tgl_Out_Jurnal BETWEEN '$start' AND '$end' ORDER BY Tanggal_Masuk DESC");
            return $sql->result();
          }if ((!empty($start)) && (empty($end))) {
            $sql=$this->db->query("SELECT * FROM dokumen where Tgl_Out_Jurnal = '$start' ORDER BY Tanggal_Masuk DESC");
            return $sql->result();
          }if ((empty($start)) && (!empty($end))) {
            $sql=$this->db->query("SELECT * FROM dokumen where Tgl_Out_Jurnal = '$end' ORDER BY Tanggal_Masuk DESC");
            return $sql->result();
          }
