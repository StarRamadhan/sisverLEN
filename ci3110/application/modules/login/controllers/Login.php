...
class Login extends MY_Controller{
  ...
  function auth(){//Proses pengecekan login
    ...
    $check=$sql->num_rows();
      ...
      if ($data['jabatan']=='admin') {//AKSES KE Admin
        $this->session->set_userdata('akses','admin');
        $this->session->set_userdata('ses_id',$data['id_user']);
        $this->session->set_userdata('ses_nama',$data['nama_user']);
        redirect('page');
      }if ($data['jabatan']=='manager') {//AKSES KE Manager
        $this->session->set_userdata('akses','manager');
        $this->session->set_userdata('ses_id',$data['id_user']);
        $this->session->set_userdata('ses_nama',$data['nama_user']);
        redirect('page');
      }if ($data['jabatan']=='jurnal') {//AKSES KE Jurnal
        $this->session->set_userdata('akses','jurnal');
        $this->session->set_userdata('ses_id',$data['id_user']);
        $this->session->set_userdata('ses_nama',$data['nama_user']);
        redirect('page');
      }if ($data['jabatan']=='verifikasi1') {//AKSES KE ADMIN
        $this->session->set_userdata('akses','verifikasi');
        $this->session->set_userdata('ses_id',$data['id_user']);
        $this->session->set_userdata('ses_nama',$data['nama_user']);
        redirect('page');
      }if ($data['jabatan']=='verifikasi2') {//AKSES KE Verifikasi 2
        $this->session->set_userdata('akses','verifikasi');
        $this->session->set_userdata('ses_id',$data['id_user']);
        $this->session->set_userdata('ses_nama',$data['nama_user']);
        redirect('page');
      }if ($data['jabatan']=='verifikasi3') {//AKSES KE Verifikasi 3
        $this->session->set_userdata('akses','verifikasi');
        $this->session->set_userdata('ses_id',$data['id_user']);
        $this->session->set_userdata('ses_nama',$data['nama_user']);
        redirect('page');
